...
\end{NB}
\begin{Df}[Distance à une partie]
...
\begin{tikzpicture}
   ...
   \end{axis}
\end{tikzpicture}
\captionof*{figure}{Représentation graphique des premier termes de la suite de fonctions $(f_n:x\to x^n)_{n\in \N   }$}
\end{Ex}
...
\end{tikzpicture}
\captionof*{figure}{Exemple de suite dans $\R^2$. Quelque soit le rayon $\epsilon$, à partir d'un certain rang $N$, tous les termes de la suite doivent appartenir à la boule $B(\vec{l},\epsilon)$,}
\end{center}
...
\begin{itemize}
\item Une \defi{partie ouverte} de $\K^n$, ou plus simplement un \defi{ouvert} de $\K^n$, est une partie $A$ de $\K^n$ telle que $$\forall \vec{x}\in A, \exists r>0:\quad B(\vec{x},r)\subset U,$$
