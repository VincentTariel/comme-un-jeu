...
La notion de vecteurs normaux sera l'objet du chapitre \textbf{espaces préhilbertiens}. 
...
\draw[dotted] (.7*\myunit,0) node {$=$};
\matrix (Y) [matrix of math nodes,%
             nodes = {node style ge},%
             left delimiter  = (,%
             right delimiter = )] at (2.2*\myunit,0)
{%
  \color{blue}2,5  \\
  \color{blue}0,5 \\
};
...
\end{pmatrix}$ et $B=\begin{pmatrix}
 ...
 0,5  \\
\end{pmatrix}$.\\
En calculant le déterminant de la matrice (voir chapitre \textbf{déterminant)}, on démontre que la matrice  $\begin{pmatrix}
 1 & 1   \\
 -1 & 1  \\
...
Les\textbf{matrices} seront étudiées dans le chapitre \textbf{matrices}. 
\section{Espace vectoriel}
Une idée est de transformer ce système deux équations à deux inconnues en un système d'une équation à deux inconnues. On a :\\
$$\begin{cases}
{\color{red}x}+{\color{green}y}&={\color{blue}2,5}\\
{\color{red}-x}+{\color{green}y}&={\color{blue}0,5}
\end{cases}
...
Il s'agit de déterminer $x,y$ afin d'exprimer $\begin{pmatrix}
 2,5   \\
 0,5  \\
\end{pmatrix}$  comme combinaisons linéaires des vecteurs $\begin{pmatrix}
 1    \\
 -1   \\
\end{pmatrix}$ et $\begin{pmatrix}
 1   \\
  1  \\
\end{pmatrix}$ (voir animation GeoGebra \url{https://www.geogebra.org/m/shsawcqc}). D'un point de vue mathématiques, on se pose trois questions :
\begin{enumerate}
\item \impo{Existence:} la famille est \impo{génératrice} de $\R^2$, c'est à dire  
pour tout vecteur de $\R^2$ il existe un couple $(x,y)$ tel que ce vecteur s'exprime comme combinaison linéaire de   $\begin{pmatrix}
 1    \\
 -1   \\
\end{pmatrix}$ et $\begin{pmatrix}
 1   \\
  1  \\
\end{pmatrix}$. Cela est vrai, en particulier pour le vecteur $\begin{pmatrix}
 2,5   \\
 0,5  \\
\end{pmatrix}$. Avec  GeoGebra, on trouve la couple $(x=1,y=1,5)$.
\item \impo{Unicité:} la famille est \impo{libre},  , c'est à dire si un vecteur de $\R^2$ s'exprime comme combinaison linéaire de   $\begin{pmatrix}
 1    \\
 -1   \\
\end{pmatrix}$ et $\begin{pmatrix}
 1   \\
  1  \\
\end{pmatrix}$ alors cette combinaison est unique. Cela est vrai, en particulier pour le vecteur $\begin{pmatrix}
 2,5   \\
 0,5  \\
\end{pmatrix}$,   on ne peut pas trouver un autre couple que $(x=1,y=1,5)$.\\
\item \impo{Existence} et \impo{Unicité} : les vecteurs $\begin{pmatrix}
 1    \\
 -1   \\
\end{pmatrix}$ et $\begin{pmatrix}
 1   \\
  1  \\
\end{pmatrix}$ forment une \impo{base} de $\R^2$, c'est à dire que pour tout vecteur de $\R^2$ il existe un unique couple $(x,y)$ tel que ce vecteur s'exprime comme combinaison linéaire de   $\begin{pmatrix}
 1    \\
 -1   \\
\end{pmatrix}$ et $\begin{pmatrix}
 1   \\
...
\end{pmatrix}$. Cela est vrai, en particulier pour le vecteur $\begin{pmatrix}
 2,5   \\
 0,5  \\
\end{pmatrix}$.  On a ainsi l'existence et l'unicité de la solution du système linéaire. \\
\end{enumerate}
L'étude des combinaisons linéaires sera fait dans le chapitre espaces vectoriels.\\
...
{y}   \\
\end{pmatrix})$$
on peut démontrer qu'elle est bijective. Ainsi, il existe une unique vecteur antécédent au vecteur $\begin{pmatrix}
 {\color{blue}2,5}   \\
 {\color{blue}0,5}  \\
\end{pmatrix}$ donc le système admet une unique solution.\\
L'étude des applications linéaires sera fait dans le chapitre \textbf{applications linéaires}.
