...
\end{itemize}
\end{Prop}
\begin{Ex}Démontrer que $F:x\mapsto \int_{0}^{+\infty} e^{-xt^2}\, \mathrm dt$ est définie sur $]0,+\infty[$.\\
...
\begin{itemize}
\item
  \textbf{Hypothèse de domination locale:}
...
\item  \textit{Domination:}\\
On a :
$$\forall x\in [a,b], \forall t\in [0,1]: \quad  \Abs{\frac{ e^{xt}}{1+t^2}}\leq M=\sup_{(x,t)\in[a,b]\times[0,1]}|\frac{ e^{xt}}{1+t^2}| .$$
