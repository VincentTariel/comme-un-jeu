...
\end{Df}
\begin{Ex}\label{ex:1}Soit les suites $(f_n)_{n\in \N   }$, $(g_n)_{n\in \N^*   }$ et $(h_n)_{n\in \N   }$ définies par :
...
\end{Df}
\begin{Ex}[Série géométrique]La série géométrique : $\sum x^n $ 
\end{Ex}
...
Pour prouver que $\fn$ converge simplement vers $f$ sur $I$, il faut fixer la variable de  la fonction, \textbf{Fixons $a\in I$},  et chercher à prouver que la \textbf{suite numérique} $(f_n(a))_{n\in \N }$ converge vers $f(a)$. Il s'agit donc d'un problème de convergence d'une  suite numérique.\\
...
\begin{Ex}[Série géométrique] Soit la série de fonctions $\sum x^n$.\\
Fixons $a\in ]-1,1[$.\\
On a $\sum_{k=0}^n a^k= \frac{1-a^{n+1}}{1-a}\Toninf  \frac{1}{1-a}$ car $|a|<1$. Donc la série converge simplement sur $]-1,1[$ et sa limite simple est la fonction $x\mapsto \frac{1}{1-x}$.
\end{Ex}
...
On définit la limite simple, $f$, de la suite $\fn$ à l'aide de la convergence simple car si la limite de convergence uniforme existe, elle est égale à celle de la convergence simple. Puis, on vérifie que le suite  $\fn$ converge uniformément vers cette limite $f$ par 
\begin{enumerate}
...
\end{proof}
\begin{Ex}Soit la série de fonctions $\sum \frac{\sin(nx)}{n^2}$.\\
On a :
...
\begin{Df}[Fonctions en escalier] Une fonction réelle définie sur un intervalle [a,b] de R est dite \defi{en escalier} s'il existe des points $a=x_0<x_1<...<x_n=b$, tels que sur chaque segment $]x_i,x_{i+1}[$, la fonction soit constante, par exemple, 
...
\end{Th}
\begin{Df}[Développement en série entière (voir le chapitre)]
