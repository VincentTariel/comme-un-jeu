...
$$y' = y ,\quad y(0)=1 (E)$$
L'unique solution sur $\R^=$ de cette équation différentielle linéaire homogène du premier ordre sans
second membre est l'application $x\to e^x$. Par définition, la fonction exponentielle est l'unique fonction égale à sa dérivée.\\
\begin{enumerate}
...
f'(x)=&\sum_{n=0}^{+\infty}(a_n x^n)' \text{ "par permutaion dérivé et somme" }\\
f'(x)=&\sum_{n=1}^{+\infty} n a_n x^{n-1} \\
f'(x)=&\sum_{n=0}^{+\infty} (n+1) a_{n+1} x^{n}\text{ "par translation d'indice" } \\
\end{align*}
Comme $f'(x)=f(x)$, on obtient :
$$\sum_{n=0}^{+\infty}a_n x^n=\sum_{n=0}^{+\infty} (n+1) a_{n+1} x^{n}.$$
Par identification, on obtient :
$$ \forall n \in \N (n + 1)a_{n+1} = a_n.$$
%Ce qui conduit rapidement à an =
%a0
%n!
%pour tout n ? N. Ainsi, si toutes les opérations effectuées sont
%licites, en prenant arbitrairement a0 = 1,
%f (x ) =
%+
%X?
%n=0
...
%n
%n!
%apparaît comme une solution de l?équation (E).
%1. Pourquoi diantre se limiter à des sommes finies...
%2 Chap. 2 Séries entières
%? L?équation (E) n?est cependant pas très mystérieuse, nous savons que ses solutions sont de la forme
%x 7? ?e
%x
%. Comme la fonction exponentielle est l?unique solution de (E) qui vérifie la condition initiale
%y (0) = 1, on trouve alors :
%?x ? R exp(x ) =
%+
%X?
%n=0
...
%n!
\end{enumerate}
Ce premier exemple soulève de nombreuses questions auxquelles nous al
...
La somme de la série $\sum_n \frac {x^{n}}{n!}$ est la fonction exponentielle :
...
\caption{La fonction exponentielle, $x\mapsto e^x$ en bleue, et la somme des $n+1$ termes $x\mapsto \sum_0^n \frac{x^n}{n!}$ en rouge. }
\end{center}
\end{figure}
