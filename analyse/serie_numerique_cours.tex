...
\begin{Df}
  ...
  La \defi{série numérique} de \defi{terme général} $u_n$ est la suite $(S_n)_{n\in\N}= (\sum_{k=0}^n u_k)_{n\in\N}$. En particulier on a $$S_0=u_0,\quad S_1=u_0+u_1,\quad S_2=u_0+u_1+u_2.$$
  $S_n$ s'appelle la \defi{somme partielle} d'ordre $n$ de la série $\SER$.
\end{Df}
\begin{Ex}
...
On dit que la série numérique $\SER$ est \defi{grossièrement divergente} si $u_n$ ne tend pas vers 0.
\end{Prop}
\begin{proof}
...
\begin{Df}
Une \defi{série à termes positifs} est une série réelle $\SER$ telle que $$\forall n\in\N,\quad u_n\geq 0.$$
\end{Df}
\begin{Prop}
...
\end{Ex}
\begin{Prop}[Règle de comparaison]
Soit $\SER$ et $\sum v_n$ deux SATP telles que \defi{$u_n \Sim_\ninf v_n$}.
Alors $\SER$ et $\sum v_n$ ont \defi{même nature}.
\end{Prop}
\begin{proof}
