...
La suite   $(S_n)_{n\in\N}$ est appelée \defi{série numérique} de \defi{terme général} et noté $\SER$.
\end{Df}
\begin{Ex}
...
\end{Df}
\begin{Prop}[Adaptation du théorème de la limite monotone]
...
Soit $\SER$ et $\sum v_n$ deux SATP telles que $u_n \Sim_\ninf v_n$.\\
Alors $\SER$ et $\sum v_n$ sont de même nature.
\end{Prop}
\begin{proof}
...
\end{proof}
\begin{Th}[Comparaison série-intégrale (version intégrale impropre)]
Soit f continue sur $[1,+\infty[$, décroissante
et positive sur cet intervalle. \\
