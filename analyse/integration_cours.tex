...
\begin{Df}[Intégrale impropre sur $[a,b[$]
...
La fonction $ x\mapsto  \frac{1}{x^2}$ est continue sur $[1,{+\infty }[$.  Soit $t\in [1,{+\infty }[$.\\
On a  $\int_1^t \frac{1}{x^2}\,\mathrm dx=[-\frac 1 x]_1^t=1-\frac{1}{t}\tend[t\to+\infty ]1$. Donc l'intégrale est convergente et son intégrale impropre est égale à 1.
\end{Ex}
\begin{Prop}[Indépendance de la borne fermée]
...
\begin{Df}[Intégrale impropre sur $\TTT$]
...
\end{Ex}
\begin{Df}[Intégrale impropre sur $\TTTT$]
...
\begin{itemize}
\item $\int_0^{1 } \frac{1}{\sqrt{x}}\,\mathrm dx$\\  Soit $t\in ]0,1]$.\\
On a  $\int_t^1 \frac{1}{\sqrt{x}}\,\mathrm dx=[2 \sqrt{x}]_t^1=2(1-\sqrt{t})\tend[t\to 0 ]2$. Donc l'intégrale est convergente.
\item $\int_1^{+\infty } \frac{1}{\sqrt{x}}\,\mathrm dx$\\ Soit $t\in [1,{+\infty }[$.\\
On a  $\int_1^t \frac{1}{\sqrt{x}}\,\mathrm dx=[2 \sqrt{x}]_1^t=2(\sqrt{t}-1)\tend[t\to +\infty ]+\infty $. Donc l'intégrale est divergente.
\end{itemize}
...
En effet soit $\int_{-\infty}^{+\infty} {x}\,\mathrm dx.$, on a:
\begin{enumerate}
\item
...
\begin{Prop}[Prolongeable par continuité]
...
\begin{Th}[Divergence grossière à l'infini]
...
\end{proof}
\begin{Prop}[Exponentielle et logarithme]
  \begin{itemize}
  \item
...
$$\int_t^1   \ln(x) \,\mathrm dx \overbrace{=}^{\text{IPP avec }u(x)=\ln(x), v'(x)=1}[\ln(x)x]_t^1- \int_t^1   \frac 1 x x \,\mathrm dx= -\ln(x)x+(t-1)\tend[t\to +\infty ] -1.$$
...
$$ \int _{1}^{t }{\frac {1}{x(x+1)}}\,\mathrm {d} x=\int _{1}^{t }\frac {1}{x}\,\mathrm {d} x-\int _{1}^{t }\frac {1}{x+1}\,\mathrm {d} x=\ln(2)+\ln(\frac{x}{x+1})\tend[t\to +\infty]\ln(2).$$
\end{Ex}
\begin{Cor}[Cas complexe]
...
\begin{Th}[Changement de variables]
...
\end{Ex}
\begin{Th}[Intégration par parties]
...
\begin{Prop}[Caractérisation de la majoration]
...
\begin{proof}
Soit $t\in[a,b[$.
$$ \int_a^t f(x)\,\mathrm dx\overbrace{\leq}^{\text{coissance sur un segment}}\int_a^t g(x)\,\mathrm dx\overbrace{\leq}^{\text{caracterisation majoration}}\int_a^b g(x)\,\mathrm dx.$$
...
\end{Ex}
\begin{Th}[Règle de petit 0 et grand O]
Soit $f,g:[a,b[\to\R$ deux fonctions continues et positives tel que
...
\end{Th}
\begin{proof}
 Comme $ f(x)=\underset{ \overset { x \rightarrow b } {} } {o}(g(x))$, il existe $t\in[a,b[$ tel que $f(x)\leq g(x), \forall x\geq t.$ Comme $\int_t^b g(x)\,\mathrm dx$ converge, $\int_t^b f(x)\,\mathrm dx$ converge par règle de comparaison. Sur le segment $[a,t]$,  $ \int_a^t f(x)\,\mathrm dx$ converge également. Finalement,  $\int_a^b g(x)\,\mathrm dx$ converge par raccordement.
\end{proof}
\begin{Ex}
...
tel que tel que $$\forall x\in [t,b[: \quad  \frac{1}{2}f(x)\leq g(x)\leq \frac{3}{2}.$$
On conclut par règle comparaison. Dans le cas négatif, la preuve est identique pour $-f$ et $-g$.
\end{proof}
...
$$\sum_{n=1}^{N-1}f(n)\leq \int_1^N f(x)\,\mathrm dx \leq \sum_{n=2}^{N}f(n).$$
Par passage à la limite, on a :
$$\sum_{1}^{+\infty}f(n)\leq \int_1^{+\infty} f(x)\,\mathrm dx \leq \sum_{2}^{+\infty}f(n).$$
On peut affirmer que série $\sum f(n)$ converge si et seulement si l'intégrale  $\int_a^{+\infty} f(x)\,\mathrm dx$ converge.
\begin{Th}[Comparaison séries/intégrales]
Soit $f$ une application continue, positive et décroissante sur $[a,+\infty[$.\\
Alors la série $\sum_n f(n)$ et l'intégrale $\int_a^{+\infty} f(x)\,\mathrm dx$ sont de même nature.
\end{Th}

Une application directe de ce théorème nous donne de nouvelles séries de référence.
\begin{Th}[Séries de Riemann]
Soit $\alpha \in \R$.\\
...
\end{Df}
\begin{Ex} Démontrons que  $\int_1^{+\infty } \frac{\sin x}{x^2}\,\mathrm dx$ est absolument convergente. On a :
$$\forall x \in [1,{+\infty }[ : \left| \frac{\sin x}{x^2}\right|\leq \frac{1}{x^2}.$$ 
Comme  $\int_1^{+\infty}\frac{1}{x^2 }\,\mathrm dx$ est convergente par intégrale de Riemann, $\int_1^{+\infty}\frac{1}{x^2 + \sqrt{x}}\,\mathrm dx$ converge, donc $\int_1^{+\infty } \frac{\sin x}{x^2}\,\mathrm dx$ est absolument convergente.
\end{Ex}
\begin{Th}
...
On a :
$$\forall n \in  \N:\quad \int_{n\pi}^{(n+1)\pi}\left|\frac{\sin x}{x}\right|\,\mathrm dx \overbrace{=}^{n\pi+u=t} \int_{0}^{\pi}\left|\frac{\sin u}{n\pi+u}\right|\,\mathrm du \geq \frac{1}{(n+1)\pi} \int_{0}^{\pi}\sin u\,\mathrm du =\frac{2}{(n+1)\pi}.$$
D'où : 
...
Soit $f : [a,b [\to \R$ continue sur $\K$ telle que $\int_a^b f(x)\,\mathrm dx$ est \defi{absolument convergente}.
\\
Alors 
$$\left| \int_a^b f(x)\,\mathrm dx \right| \leq  \int_a^b \left| f(x) \right| \,\mathrm dx.$$
\end{Prop}
\begin{proof}
Comme $\int_a^b f(x)\,\mathrm dx$ est \defi{absolument convergente},  $\int_a^b f(x)\,\mathrm dx$ et  $\int_a^b -f(x)\,\mathrm dx$ sont convergentes. On a :
