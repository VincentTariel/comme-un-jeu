...
réelle, définie à partir d'une fonction de deux variables ? la variable d'intégration et le paramètre ? par intégration sur un ensemble fixe par rapport à la variable d'intégration :
...
En seconde partie,  nous verrons le cas où le paramètre est un réelle et nous introduirons des théorèmes pour démontrer la continuité de la fonction $F$  et la permutation de la dérivée et  l'intégrale $(\frac{dF}{dx}(x)=\int_J \frac{\partial f}{\partial x }(x,t) \mathrm dt)$  sur $J$ un intervalle quelconque.
...
\begin{Th}[Permutation limite/intégrale]
Soit $\fn$ une suite de fonctions de $[a,b]$ dans $\K$.
On suppose que:
\begin{itemize}
\item
...
\end{Th}
\begin{Th}[Permutation somme/intégrale]
...
\begin{Th}[Convergence dominée] Soit $\fn$ une suite de fonctions continues par morceaux de $J$ dans $\K$, et $f,\phi:J\to\K$ continues par morceaux avec $\phi $ positive. On suppose que
\begin{enumerate}
\item \textbf{Convergence simple de $(f_n)$ vers $f$} : pour tout $t\in J$, la suite numérique $(f_n(t))$ converge vers $f(t)$;
\item \textbf{Hypothèse de domination} : pour tout $t\in J$ et tout $n\geq 1, |f_n(t)|\leq\phi (t)$,  ;
\item \textbf{Intégrabilité de la fonction dominante} :  la fonction $\phi $ est intégrable sur $J$.
\end{enumerate}
Alors toutes les fonctions $f_n$ et $f$ sont intégrables sur $J$, et on a :
$$\lim_{n\to +\infty}\int_J |f_n(t)-f(t)| \mathrm dt=0.$$
En particulier, 
...
doit majorer chaque $|f_n(t)|$ par une expression \textbf{dépendante de $t$ et indépendante de $n$} et qui soit une fonction de $t$
intégrable sur $J$. 
\end{NB}
...
Soit $t>1$ fixé.\\
La suite numérique $(\frac{1}{1+t^n})_{n\in\mathbb{N}}$ converge vers 0. Donc la suite de fonctions $(x\mapsto(\frac{1}{1+t^n}))_{n\in\mathbb{N}}$ converge simplement vers la fonction nulle $x\mapsto 0$.
...
\begin{Th}[Intégration terme à terme] Soit $\Sfn$ une série de fonctions continues par morceaux de $J$ dans $\K$, et $f:J\to\K$ continue par morceaux. On suppose que
\begin{enumerate}
\item \textbf{Convergence simple de $\Sfn$ vers $f$}: pour tout $t\in J$, la suite numérique $\sum_n f_n(t)$ converge vers $f(t)$;
\item \textbf{Hypothèse de domination} : la série numérique $\sum_n \int_J|f_n(t)|dt$ converge.
\end{enumerate} 
...
\begin{Ex} Démontrer que $\sum_{n=0}^{+\infty}\int_0^1 t^{2n}(1-t)\mathrm dt=\ln(2)$.
\begin{enumerate}
\item \textit{Convergence simple sur $[0,1[$:}\\ 
La série géométrique $\sum_n t^{2n}$ converge simplement vers $t\mapsto \frac{1}{1-t^2}.$ Donc la  série $(1-t)\sum_n t^{2n}$ converge simplement vers $t\mapsto \frac{1}{1-t}.$.
\item \textit{Hypothèse de domination :}\\ 
...
\begin{Prop} La fonction $F$ est définie sur $I$ si,
$\forall x\in \mathbb{R}$ fixé, l'intégrale $\int_J f(x,t) \mathrm dt $ converge. 
\end{Prop}
\begin{Ex}Démontrer que $F:x\mapsto \int_{0}^{+\infty} e^{-xt^2} \mathrm dt$ est définie sur $\mathbb{R}^{+*}$.\\
Soit $x\in \mathbb{R}^{+*}$ fixé. La fonction $t\mapsto e^{-tx}$ est continue et positive sur $[0,+\infty[$.\\
En $+\infty$, on a : $$e^{-xt^2}={\underset {\overset {x\rightarrow \infty }{}}{o}}(\frac{1}{t^2}).$$
Comme $\int_{1}^{+\infty} \frac{1}{t^2}\mathrm dt$ converge, par théorème de comparaison, $\int_{1}^{+\infty}e^{-tx} \mathrm dt$ converge. Par raccordement, $\int_{0}^{+\infty} e^{-xt^2} \mathrm dt$ converge, donc $F$ est bien définie sur  $\mathbb{R}$.
\end{Ex}
\subsection{Continuité}

\begin{Th}[continuité sous le signe $\int$]
On suppose:
\begin{enumerate}
\item
...
\item
  ...
  \begin{itemize}
  \item
    pour tout $(x,t)\in I×J$, on a $\Abs{f(x,t)}\leq\phi (t)$,
  \item
...
\end{itemize}
\end{Th}
\begin{Ex}Démontrer que $F:x\mapsto \int_{0}^{+\infty} e^{-xt^2} \mathrm dt$ est continue sur $[a,b]$ avec $b>a>0$.\\
\begin{enumerate}
...
On a :
$$\forall x\in [a,b], \forall t\in [0,+\infty[: \quad  \Abs{e^{-xt^2}}\leq e^{-at}.$$
...
\end{Ex}
\begin{Th}[continuité sous le signe $\int$ version locale]
...
\item
  ...
  \begin{itemize}
  \item
    pour tout $(x,t)\in K×J$, on a $\Abs{f(x,t)}\leq\phi _K(t)$,
  \item
...
\begin{Th}[Dérivation sous le signe $\int$]
On suppose:
\begin{enumerate}
\item
...
Démontrer que $F$ est de classe $\mathcal{C}^1$ sur $\R$ et vérifie, pour tout $x\in\R, F'(x)=\frac{-x}{2a}F(x).$. Déterminer l'expression de $F$.\\
\begin{enumerate}
...
\item  \textit{Domination :}\\
On a :
$$\forall x\in \R, \forall t\in \R: \quad  \Abs{-ite^{-itx}e^{-at^2}}\leq |t|e^{-at^2}.$$
...
$$F'(x)=\left[\frac{i}{2a}e^{-itx}e^{-at^2} \right]_{-\infty}^{+\infty}- \int^{+\infty}_{-\infty}\frac{i}{2a}e^{-at^2} (-ixe^{-itx} \mathrm dt.$$
Soit
$$F'(x)=\frac{-x}{2a}F(x).$$
...
\item
  ...
  \begin{itemize}
  \item
    pour tout $(x,t)\in K×J$, on a $\Abs{\frac{\partial f}{\partial x} (x,t)}\leq\phi _K(t)$,
  \item
    $\phi _K$ est continue par morceaux et intégrable sur $J$.
  \end{itemize}
\end{itemize}
...
\begin{Th}[Extension aux fonctions de classe $\mathcal{C}^p$]
On suppose:
\begin{enumerate}
\item
...
\begin{itemize}
\item
  La fonction $F \colon x \mapsto \int_J f(x,t) \mathrm dt$ est définie et de classe $\CC p$ sur $I$.
\item
