...
\begin{enumerate}
\item $X(\Omega)\times Y(\Omega)=\{x_i:i\in I\}\times \{y_j:j\in J\} $, les valeurs prises par le couple
\item $\forall (i,j)\in I\times J:\quad P(X=x_i\cap Y=y_j)$,
\end{enumerate}
On note $p_{i,j}=P(X=x_i\cap Y=y_j)$.\\
Les événements $((X=x_i)\cap (y=y_j))_{\forall (i,j)\in I\times J}$ forment un système complet d'événements de $\Omega$. En particulier, on a : 
$$\sum_{i=1}^{+\infty} \sum_{j=1}^{+\infty}P(X=x_i\cap Y=y_j)=\sum_{i=1}^{+\infty} \sum_{j=1}^m{+\infty}p_{i,j}=1.$$ 
\end{Df}
...
$$\forall i\in I: \quad P(X=x_i)=\sum_{j=1}^{+\infty}P\left((X=x_i)\cap (Y=y_j)\right)$$
et 
$$\forall j\in J: \quad P(Y=y_j)=\sum_{i=1}^{+\infty }P((X=x_i)\cap (Y=y_j)).$$ 
\end{Prop}
\begin{Ex}[Deux piles]
...
$$\forall i\in I:\quad  P_{Y=y_j}(X=x_i)=\frac{P(X=x_i\cap Y=y_j) } {P(Y=y_i)}$$ 
\item On appelle \defi{loi conditionnelle} de $Y$ sachant $(X = x_i )$ pour $P(X = x_i )\neq 0$, la probabilité définie par  :
$$\forall j\in J:\quad  P_{X=x_i}(Y=y_j)=\frac{P(X=x_i\cap Y=y_j) } {P(X=x_i)}$$ 
\end{enumerate}
\end{Df}
\begin{NB}
Les lois marginales et les lois conditionnelles déterminent la loi conjointe :
$$\forall (i,j)\in I\times J:\quad P(X=x_i\cap Y=y_j)=P_{X=x_i}(Y=y_j)P(X=x_i)$$
...
\begin{Df}[Espérance mathématique]
Soit $X$ une variable aléatoire discrète sur l'espace probabilisé $(\Omega,\mathcal{A},P)$. On énumère l'ensemble $X (\Omega) = \{x_n | n \in \N\}$.
...
\begin{Th}[Fonction génératrice de la somme de deux variables indépendantes]
