...
Soit $(\Omega,\mathcal{P}(\Omega)$ un espace probabilisé et $X$ une variable aléatoire réelle sur cette espace.\\
...
\end{align*}
\end{Df}
Comme toutes les variables aléatoires sont supposées finies, on s'autorise désormais à écrire l'ensemble des images sous la forme $X (\Omega) = \{x_n,x_2,\dots,x_n\}$.
...
L'application
\[ \Fonction{P_X}{\mathcal{P}(\R)}{[0,1]}{A}{P(X^{-1}(A))} \]
...
\end{Prop}
\begin{proof}
La démonstration repose sur $$\forall A \in \mathcal{P}(E): \quad P(X\in A)=P(\underbrace{\bigcup_{x\in A\cap X(\Omega)  }}_{disjoints} (X=x))=\sum_{x\in A}P(X=x).$$
\end{proof}
\begin{Prop}
\begin{itemize}
...
\begin{center}
\includegraphics[width=6cm]{somme.png}
\end{center}
\end{Ex}


\begin{Th}
Soit $\{x_1,\dots,x_n\}$ une famille finie de réels et $\{p_1,\dots,p_n\}$ une famille de réels positifs telle que $\sum_{i=1} p_i = 1$. Alors il existe 
une variable aléatoire  $X$ sur un
espace probabilisé $(\Omega ,\mathcal{P}(\Omega) ,P)$ à valeurs dans  $\{x_1,\dots,x_n\}$ telle que : 
...
\end{tikzpicture}
\end{center}
On a $$\forall j\in \{1,\dots,m\}:\quad P(f(X)=y_j)=\sum_{i\in\{1,\dots,n \} \text{ tel que }f(x_i)=y_j }P(X=x_i)$$  
\end{Df}
\begin{Ex}[Jeu]
...
$$\forall i\in \{1,\dots,n\}: P(X=x_i)=\sum_{j\in \{1,\dots,m\} }P((X=x_i)\cap (Y=y_j))$$
et 
$$\forall j\in \{1,\dots,m\}: P(Y=y_j)=\sum_{i\in \{1,\dots,n\}}P((X=x_i)\cap (Y=y_j)).$$ 
