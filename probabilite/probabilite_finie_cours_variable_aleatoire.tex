...
Le théorème permet surtout 
définir une variable aléatoire par sa loi de probabilité sans avoir à étudier l'expérience aléatoire sous-jacente.
\begin{Th}
...
\begin{enumerate}
\item On appelle \defi{loi conditionnelle} de $X$ sachant $(Y = y )$ pour $P(Y = y )\neq 0$, la probabilité définie par  :
$$\forall x\in X (\Omega):\quad  P_{Y=y}(X=x)=\frac{P(X=x\cap Y=y) } {P(Y=y)}$$ 
\item On appelle \defi{loi conditionnelle} de $Y$ sachant $(X = x )$ pour $P(X = X )\neq 0$, la probabilité définie par  :
