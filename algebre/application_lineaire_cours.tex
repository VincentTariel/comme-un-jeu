...
\end{Prop}
\begin{Prop}[Base canonique] La dimension de $\MnpK$ est  $n p$. La base canonique est $(E_{i,j})_{1\leqslant i\leqslant n, 1\leqslant j\leqslant p}$. La matrice  $E_{i,j}$ est celle dont tous les coefficients sont nuls sauf celui d'indice $(i,j)$ , qui vaut 1. Les coordonnées dans la base canonique d'une matrice $A$ sont ses coefficients :$$ A = \sum _{1\leqslant i\leqslant n\atop 1\leqslant j\leqslant p}  a_{i,j} E_{i,j}.$$
On a $\dim( \MnpK)=np$.
\end{Prop}
...
L'espace vectoriel des endomorphismes de \defi{$\mathcal{L}(E)$} se note $\LE = \mathcal{L}(E,E)$.
\end{Df}
...
\[u(\vec{e_j})=a_{1,j}.\vec{f_1}+\dots +a_{n,j}.\vec{f_n}. \]
...
Alors
\[  [v?u]_{\mathcal{B}}^{\mathcal{B}''} = [v]_{\mathcal{B}'}^{\mathcal{B}''} ×[u]_{\mathcal{B}}^{\mathcal{B}'}. \]
\end{Th}
...
\begin{center}
$u$ est bijective $\Leftrightarrow$ $u$ est surjective $\Leftrightarrow$ $u$ est surjective.
\end{center}
...
$\exists v\in \mathcal{L}(E): u\circ v =v \circ u = \mathrm{Id}_E$ &$\Leftrightarrow$ &  $\exists v\in \mathcal{L}(E): u\circ v= \mathrm{Id}_E$  &$\Leftrightarrow$ &  $\exists v\in \mathcal{L}(E): v\circ u = \mathrm{Id}_E$
\end{tabular}
\end{center}
\end{Prop}
\begin{Ex}
Soit $u:(x,y)\to (x+y,x-y)$ est bijective.\\
Comme $u$ est un endomorphisme, il suffit montrer qu'elle est injective.\\
$$(x,y)\in \Ker u \Leftrightarrow \begin{cases}x+y=0\\x-y=0\end{cases}  \Leftrightarrow  \begin{cases}x=0\\y=0\end{cases} $$ 
Donc $\Ker u =\{(0,0)\}.$
\end{Ex}
\begin{Ex}
Soit $u:P\to P-P'$ est bijective sur $\R_n[X]$.\\
Comme $u$ est un endomorphisme, il suffit montrer qu'elle est injective.\\
...
bases, il faut savoir déterminer celle du même dans deux autres bases.
...
Si  $P$ est inversible alors $P$ est une matrice de passage de la base canonique à la base de ses colonnes $(C_1,\dots, C_n)$.
\end{Prop}
...
\end{center}
L'idée fondamentale est de déterminer l'application  déterminant à partir de ses propriétés  dans le plan.\\ 
...
\end{Prop}
\begin{Prop}[Matrice triangulaire] Le déterminant d'une matrice triangulaire,$\begin{pmatrix}a_{1,1}&a_{1,2}&\cdots &\cdots &a_{1,n}\\0&a_{2,2}&&&a_{2,n}\\\vdots &\ddots &\ddots &&\vdots \\\vdots &&\ddots &\ddots &\vdots \\0&\cdots &\cdots &0&a_{n,n}\\\end{pmatrix}$,  est égale au produit des coefficients de la diagonale, soit $a_{1,1}\times a_{2,2} \times\dots\times a_{n,n}$.
\end{Prop}
...
$$u(\vec{e_j})=a_{1,j}.\vec{f_1}+\cdots +a_{n,j}.\vec{f_n}\text{ avec } 1\leq j \leq p.$$
...
Substituons $u(\vec{e_j})$ par $a_{1,j}.\vec{f_1}+\cdots +a_{n,j}.\vec{f_n}$ dans l'équation précédente, ce qui donne après réorganisation des termes :
\begin{align}
u(\vec{x})=& (a_{1,1}.\lambda_1+\dots+a_{1,p}.\lambda_p).\vec{f_1}+\cdots \\
        &+(a_{n,1}.\lambda_1+\dots+a_{n,p}.\lambda_p).\vec{f_n}.
\end{align}        
Donc, l'application linéaire $u$ est entièrement déterminée par les coefficient $a_{i,j}$. On range les coordonnées du vecteur $u(\vec{x})$  dans un vecteur colonne $\M{M}{n,1}{\K}$:      
$$\left[u(\vec{x})\right]_{\mathcal{B}_F}=\begin{pmatrix}a_{1,1}.\lambda_1+\dots+a_{1,p}.\lambda_p\\ \vdots\\a_{n,1}.\lambda_1+\dots+a_{n,p}.\lambda_p\end{pmatrix}.$$
...
\begin{pmatrix}
    ...
    \vdots &   &  \vdots  \\
    c_{n,1} &  \dots & c_{n,m}
  \end{pmatrix}& &=\begin{pmatrix}c_{1,j}\\\vdots\\c_{n,j}\end{pmatrix}
\end{align*}
...
\begin{pmatrix}
    ...
    \vdots &   &  \vdots  \\
    a_{n,1} &  \dots & a_{n,p}
  \end{pmatrix}& &=\begin{pmatrix}a_{1,j}\\\vdots\\a_{n,j}\end{pmatrix}
\end{align*}
puis la matrice $B$,
\begin{align*}
&\begin{pmatrix}a_{1,j}\\\vdots\\a_{n,j}\end{pmatrix}&\\   
\begin{pmatrix}
    b_{1,1} &  \dots & b_{1,n}  \\
    \vdots &   &  \vdots  \\
    b_{m,1} &  \dots & b_{m,n}
  \end{pmatrix}& &=\begin{pmatrix}b_{1,1}a_{1,j}+\cdots +b_{1,n}a_{n,j}\\\vdots\\b_{m,1}a_{1,j}+\cdots +b_{m,n}a_{p,n}\end{pmatrix}
\end{align*}
par identification, on obtient $c_{i,j}=b_{i,1}a_{1,j}+\cdots +b_{i,n}a_{n,j}=\sum _{k=1}^{n}b_{i,k}a_{k,j}$, soit la multiplication terme à terme des coefficients de la  i-ème ligne de $B$ et de j-ème colonne de $A$,et la somme de ces produits. Un schéma de ce produit est :  
\begin{center}
