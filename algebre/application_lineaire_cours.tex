...
\begin{Df}[Coefficient de proportionnalité]
Traduisant la proportionnalité, une fonction $u:\mathbb{R}\mapsto\mathbb{R}$ est dite linéaire si  $ u:x\mapsto a x$ où  $\lambda$ est appelé \defi{coefficient de proportionnalité}.
\end{Df}
\begin{Ex}[Plein d'essence]
Le prix à la pompe est une fonction linéaire du volume d'essence mis dans le réservoir.
\end{Ex}
\begin{Ex}[Différentielle]
...
\end{proof}
\begin{Prop}[Base canonique]La \defi{base canonique} est $(E_{i,j})_{1\leqslant i\leqslant n, 1\leqslant j\leqslant p}$ où la matrice  $E_{i,j}$ est celle dont tous les coefficients sont nuls sauf celui d'indice $(i,j)$ , qui vaut 1. Les coordonnées dans la base canonique d'une matrice $A$ sont ses coefficients :$$ A = \sum _{1\leqslant i\leqslant n\atop 1\leqslant j\leqslant p}  a_{i,j} E_{i,j}.$$
On a $\dim( \MnpK)=np$.
\end{Prop}
\begin{proof}
...
\begin{DfProp}[Application linéaire]
...
\end{Ex}
\begin{DfProp}[Espace vectoriel des application linéaires et endomorphismes]
Soit $E$ et $F$ deux $\K $-espaces vectoriels.
...
\end{Th}
\begin{proof}[Caractérisation par l'image d'une base]
...
\end{pmatrix} \]
\end{Df}
\begin{NB}[Identification matrice colonne et $p$-uplets]
...
\begin{DfProp}[Application linéaire d'une matrice]
