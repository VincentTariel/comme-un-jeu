...
On note $\SOnR$ l'ensemble des matrices orthogonales directes de $\MnR$ et $\SOE$ l'ensemble des rotations de $E$.
\end{Df}
\begin{Ex}
...
\item \textit{Stabilité} :  Soit $S_1,S_2\in \SOnR$. $$\det (S_1 S_2^{-1})=\det S_1 \det S_2^{-1}=\det S_1 \det S_2=1.$$
\end{itemize}
\end{proof}
...
\item angle :
\begin{itemize}
\item  $\tr(A)=2$, et donc $\cos ? =\frac 1 2$, soit $?=\pm \frac \pi 3[2?]$. Pour déterminer le signe de $\theta$, on pose $\vec{y}=(1,?1,0)$ non colinéaire  $\vec{x}$. $f(\vec{y})=(1,0,-1)$ et $\det (\vec{x},\vec{y},r(\vec{y}))=3>0$. ce qui signifie $\theta?\in]0,\pi[ modulo 2\pi.$ On en déduit donc que $f$ est la rotation d'axe dirigé par $(1,1,1)$ et d'angle $\frac \pi 3$.
\end{itemize}
\end{itemize}
\end{Ex}
...
Il s'agit d'un sous-espace vectoriel de $\MnR$ et $\dim \SnR=  \frac{n(n+1)}{2}$.
\end{Df}
...
Soit $A\in \MnR$. Alors $\transposee{A} A$ est symétrique.\\
\end{Ex}
...
Il s'agit d'un sous-espace vectoriel de $\MnR$ et $\dim \AnR=  \frac{n(n-1)}{2}$.
\end{Df}
