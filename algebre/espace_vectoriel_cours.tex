...
%\Para{Corollaire}
%
%Soit $E$ un £evdf. et $(\vec{e}_1,\dots,\vec{e_p})$ et $(\vec{f}_1,\dots,\vec{f_q})$ deux bases de $E$.
%Alors $p=q$.
%
%\Para{Proposition-Définition}
%
%La \textbf{dimension} d'un £evdf. est égale au cardinal d'une base quelconque de $E$.
%
%\Para{Critère}
...
%
%\Para{Exemple} Pour tout polynôme $P$ appartenant à $\K _{n}[X]$, la combinaison linéaire des polynômes de Lagrange  $\sum _{j=0}^{n}P(x_{j})l_{j}(X)$ avec $x_{0},\ldots ,x_{n}$ $n + 1$ scalaires distincts  est égale au polynôme $P$ aux points $x_{0},\ldots ,x_{n}$, donc égal à $P$. Les polynômes de Lagrange forment une famille génératrice de $n+1$ vecteurs. Comme $\dim(\K _{n}[X])=n+1$, ils forment une base de $\K _{n}[X]$. 
%
%
%\Para{Proposition}
%
...
%\Para{Définition}
%
%Soit $E$ un $\K $-£evdf. $n$ et $F$ un £sev. de $E$.
%La base $\B = (\vec{e}_1,\dots,\vec{e_n})$ est dite \textbf{adaptée} à $F$
%si et seulement s'il existe $p\in \Dcro{0,n}$
%tel que $(\vec{e}_1,\dots,\vec{e_p})$ soit une base de $F$.
%
%\Para{Exemple}  La base $((1,-1,0),(0,1,-1),(0,0,1)$ est adapté à $F=\{(x,y,z)\in\R ^3: x+y+z=0\}$ dans l'espace vectoriel $\R ^3$ car $((1,-1,0),(0,1,-1))$ est une base de $F$. 
%
%
%
%\Para{Définition-Proposition}
%
%Soit $E$ un $\K $-£evdf., $\Uplet{F_1}{F_p}$ des sous-espaces vectoriels supplémentaires de $E$ et $\B$ une base de $E$.
%La base $\B$ est dite \textbf{adaptée} à la décomposition $E =?_{k=1}^p F_k$ si et seulement si $\B$ peut s'écrire comme la concaténation de $\Uplet{\B_1}{\B_p}$ où $\B_k$ est une base de $F_k$ pour tout $k\in \Dcro{1,p}$. La base ainsi définie existe.
%
%
%\Para{Proposition}
%
%Soit $E$ un $\K $-£evdf. et $\B$ une base de $E$.
%On suppose que $\B$ s'écrit comme la concaténation de $\Uplet{\B_1}{\B_p}$.
%Pour $k\in \Dcro{1,p}$, notons $F_k$ le £sev. engendré par $\B_k$.
...
%L'espace vectoriel produit $\prod_{k=1}^p E_k$ est de dimension finie £ssi. $\forall k\in \Dcro{1,p}$, $E_k$ est de dimension finie.
%De plus, dans ce cas, \[ \dim \Pa{ \prod_{k=1}^p E_k } = \sum_{k=1}^p \dim E_k. \]
%
%
%\Para{Proposition}
%Soit E un $\K $-espace vectoriel dimension finie et $F_1$, $F_2$ deux s£sev. de $E$. On a :
...
%\[ \dim\Pa{ \sum_{k=1}^p F_k } ? \sum_{k=1}^p \dim(F_k). \]
%De plus, il y a égalité si et seulement si la somme $\sum_{k=1}^p F_k$ est directe.
%
%\Para{Proposition}
%
%Soit $E$ un $\K $-£evdf. et $F_1, \dots, F_p$ des £sevs. de $E$.
...
%  $\sum_{k=1}^p F_k = E$;
%\item
%  tout vecteur $x\in E$ se décompose sous la forme
%  $x = \sum_{k=1}^p f_k$
%  où $f_1\in F_1$, $\dots$, $f_p\in F_p$,
