...
\begin{Df}[Sous espace vectoriel]
...
$$ \forall \vec{x_1} \in F_1, \forall \vec{x_2} \in F_2:\quad 
...
Supposons que la somme $F_1+F_2$ est directe.
\begin{itemize}
\item \underline{$\{\vec{0_E}\}   \subset  F_1\cap F_2$} :\\
$\vec{0_E}\in F_1,F_2$ donc $\vec{0_E}\in F_1\cap F_2 .$ 
\item \underline{$F_1\cap F_2  \subset \{\vec{0_E}\} $} :
...
\begin{itemize}
\item \underline{$\R_n[X]=\R_{n-1}[X]+ Vect(X^n) $} :\\
...
\item \underline{$\R_{n-1}[X]\cap  Vect(X^n)=\{\vec{0}_{\R_n[X]}\}$ :}\\
Soit $P \in \R_{n-1}[X]\cap Vect(X^n)$.\\
Comme $P \in \R_{n-1}[X]$, on a $deg(P)<n$. Comme  $P \in Vect(X^n)$, on a $P=\lambda X^n$. Si $\lambda\neq 0$, on a $deg(P=\lambda X^n)=n$, d'où une contradiction.  Donc $P=0$.  
\end{itemize}
\end{Ex}
\begin{Df}[Somme]
...
\end{Df}
\begin{Ex} La famille  $\{(1,1), (0,1), (1,-1)\}$  est génératrice de $\R^2$. En effet, soit $\vec{x}=(x,y)\in\R^2$, on a :
...
\begin{enumerate}
\item Puisque $\mathcal{G}$ engendre $E$, il existe un vecteur $\vec{g}$ de $\mathcal{G}$ qui n'est pas une combinaison linéaire d'éléments de $\mathcal{L}$. Nécessairement, $\vec{g}$ n'appartient pas à $\mathcal{L}$.
\item On remplace $\mathcal{L}$ par $\mathcal{L}\cup \{\vec{g} \}$, qui est encore libre car le nouveau vecteur n'est pas une combinaison linéaire des précédents.
\end{enumerate}
...
\end{Prop}
\begin{proof}
Comme $F$ est un espace vectoriel de dimension finie, il existe une base $(\vec{e}_1,\dots,\vec{e_p})$ de $F$. Cette famille est libre dans $E$. On la complète en une base de $E$ d'après le théorème de la base incomplète.  
\end{proof}
\begin{Df}
...
Pour $k\in \{1,\dots,p\}$, notons $F_k$ le sous espace vectoriel engendré par $\mathcal{B}_k$.\\
...
L'espace vectoriel produit $\prod_{k=1}^p E_k$ est de dimension finie si et seulement si $\forall k\in \{1,\dots,p\}$, $E_k$ est de dimension finie.\\
De plus, dans ce cas, \[ \dim( \prod_{k=1}^p E_k ) = \sum_{k=1}^p \dim(E_k). \]
\end{Prop}
\begin{proof}
On suppose $p=2$.\\
Soit $(\vec{e_1},\dots,\vec{e_n})$ une base de $E_1$ et $(\vec{f_1},\dots,\vec{f_q})$ une base de $E_2$.\\
Alors $( (\vec{e_1},\vec{0}_F),\dots,(\vec{e_n},\vec{0}_F),(\vec{0}_E,\vec{f_1}),\dots,(\vec{0}_E,\vec{f_q}))$ est une base de $E_1\times E_2.$
\begin{itemize}
\item \underline{Génératrice} :\\
Soit $(\vec{x},\vec{y})\in E_1\times E_2$. \\
Comme $\vec{x}\in E_1$ et $(\vec{e_1},\dots,\vec{e_n})$ est une famille génératrice de $E_1$, il existe $\lambda_1,\dots,\lambda_n\in \K$ tel que $\vec{x}=\lambda_1\vec{e_1}+\dots+ \lambda_n\vec{e_n}$.\\
Comme $\vec{y}\in E_2$ et $(\vec{f_1},\dots,\vec{f_q})$ est une famille génératrice de $E_2$, il existe $\beta_1,\dots,\beta_q\in \K$ tel que $\vec{y}=\beta_1\vec{f_1}+\dots+ \beta_q\vec{f_q}$.\\
D'où $(\vec{x},\vec{y})=\lambda_1(\vec{e_1},\vec{0}_F)+\dots+\lambda_n(\vec{e_n},\vec{0}_F)+ \beta_1(\vec{0}_E,\vec{f_1})+\dots+\beta_q(\vec{0}_E,\vec{f_q}).$
\item \underline{Libre} :\\
Soit  $\lambda_1,\dots,\lambda_n,\beta_1,\dots,\beta_q\in K$ tel que $$\lambda_1(\vec{e_1},\vec{0}_F)+\dots+\lambda_n(\vec{e_n},\vec{0}_F)+ \beta_1(\vec{0}_E,\vec{f_1})+\dots+\beta_q(\vec{0}_E,\vec{f_q})=(\vec{0}_E,\vec{0}_F).$$
D'où:
$$(\lambda_1\vec{e_1}+\dots+ \lambda_n\vec{e_n},\beta_1\vec{f_1}+\dots+ \beta_q\vec{f_q} )=(\vec{0}_E,\vec{0}_F).$$
Par identification, on obtient $\lambda_1\vec{e_1}+\dots+ \lambda_n\vec{e_n}=\vec{0}_E$ et $\beta_1\vec{f_1}+\dots+ \beta_q\vec{f_q}=\vec{0}_F$. Comme $(\vec{e_1},\dots,\vec{e_n})$ une famille libre de $E_1$ et $(\vec{f_1},\dots,\vec{f_q})$ de $E_2$ , $\lambda_1=\dots=\lambda_n=\beta_1=\dots=\beta_q=0$. 
\end{itemize}
\end{proof}
...
\end{Prop}
\begin{proof}
Une idée est de remarquer l'analogie avec cette formule ensembliste  :
$$\text{card}(A)+\text{card}(B)=\text{card}(A\cup B)+\text{card}(A\cap B).$$
Soit $C$ une base de $F\cap G$. On la complète en une base $A$ de $F$ et en une base $B$ de $G$.\\
$A\cup B$ est une base   $F+G$ et $A\cap B=C$ est une base de $F\cap G$.  
\end{proof}
\begin{Prop}
